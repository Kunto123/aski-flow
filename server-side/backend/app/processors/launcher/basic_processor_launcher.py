...
class BasicProcessorLauncher(AbstractTopologicalProcessorLauncher):
    ...
    def launch_processors(self, processors):
        ...
        # Tag the current greenlet with the session so any transform streams
        # created by processors are attributed to this session (RV-002).
        set_current_run_session(session_id)
        try:
