...
class ProcessorFactoryModule(Module):
    def configure(self, binder: Binder):
        if is_mock_env():
            # Import hanya saat mock env, supaya runtime normal tidak bergantung pada folder tests
            try:
                from tests.utils.processor_factory_mock import ProcessorFactoryMock
            except ModuleNotFoundError as e:
                raise ModuleNotFoundError(
                    "USE_MOCK=true tapi modul tests tidak ditemukan. "
                    "Pastikan folder 'tests' ada, atau matikan USE_MOCK."
                ) from e
