import json

try:
    import cv2
except Exception:
    cv2 = None

from ..processor import BasicProcessor
from ....streaming import get_stream_manager

from .media_ref_utils import extract_stream_id


class FaceRecognitionProcessor(BasicProcessor):
    """Face Recognition node (Week 6 target) - dummy skeleton for Week 5.

    For now this is a passthrough filter that preserves the media reference so
    users can chain nodes without breaking the flow.
    """

    processor_type = "face-recognition"

    def __init__(self, config):
        super().__init__(config)
        self.input_url = config.get("input_url")

    def process(self):
        input_ref = self.get_input_by_name("input_url", self.input_url)
        if not input_ref:
            raise ValueError("face-recognition requires input_url")

        stream_id = extract_stream_id(input_ref)
        if stream_id:
            return self._process_stream(stream_id)

        payload = {
            "mode": "file",
            "status": "dummy",
            "message": "Face recognition will be implemented in Week 6.",
        }
        return [input_ref, json.dumps(payload)]

    def _process_stream(self, source_stream_id: str):
        if cv2 is None:
            raise RuntimeError("opencv-python is required for face-recognition stream mode")

        manager = get_stream_manager()

        def _transform(frame):
            # Passthrough for now.
            return frame

        out_stream_id = manager.create_transform_stream(source_stream_id, _transform)
        payload = {
            "mode": "stream",
            "status": "dummy",
            "message": "Face recognition will be implemented in Week 6.",
        }
        return [
            f"stream://{out_stream_id}",
            manager.build_mjpeg_url(out_stream_id),
            json.dumps(payload),
        ]

    def cancel(self):
        pass
