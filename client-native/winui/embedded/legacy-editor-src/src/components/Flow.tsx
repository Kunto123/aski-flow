...
const Flow = forwardRef((props: FlowProps, ref) => {
  ...
  const onError = useCallback((data: FlowOnErrorEventData) => {
    ...
    if (failingNodeName) {
      ...
      clearPendingProgressTimer(failingNodeName);
    }
    setCurrentNodesRunning((previous) => {
      if (!failingNodeName) return previous;
      return previous.filter((node) => node != failingNodeName);
    });
    ...
    if (failingNodeName) {
      setNodes((prevNodes) => {
        return prevNodes.map((node: Node) => {
          if (node.data.name != failingNodeName) {
            return node;
          }
          return {
            ...node,
  ...
  const handleUpdateNodeData = useCallback((nodeId: string, data: any) => {
    setNodes((prevNodes) => {
      ...
      const isNodeRunning = currentNodesRunning.includes(String(prevNode.data?.name || ""));
      if (isNodeRunning) {
        // Runtime fields are controlled by socket progress/error events while running.
        // Keep latest runtime data so form edits do not wipe in-flight outputs.
        nextData.outputData = prevNode.data?.outputData;
        nextData.lastRun = prevNode.data?.lastRun;
        nextData.isDone = prevNode.data?.isDone;
      }
