...
const GenericNode: React.FC<GenericNodeProps> = React.memo(
  ({ data, id, selected, nodeFields, iconComponent }) => {
    ...
    useEffect(() => {
      ...
      const triggerAutoRun = runNodeIfIdle ?? runNode;
      if (!triggerAutoRun) return;
      ...
      const processingParams = isImageProcessing
        ...
        : JSON.stringify({
            ...
            stream_fps: data.stream_fps ?? 30,
            inference_fps: data.inference_fps ?? 30,
            imgsz: data.imgsz ?? 0,
      ...
      try {
        const dispatched = triggerAutoRun(data.name);
        // Only mark the key as consumed when the run was actually dispatched.
        // If the system was busy and rejected the request, the key stays stale
